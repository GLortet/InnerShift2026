...
def _match_terms(sentence: str, lexicon: set[str]) -> int:
    ...
    for term in lexicon:
        ...
        if not lowered_term:
            continue
        if len(lowered_term) < 3 and " " not in lowered_term:
            continue
        if " " in lowered_term:
...
def extract_keywords(cleaned_text: str, filtered_tokens: list[str]) -> list[dict[str, Any]]:
    ...
    if cleaned_text and TfidfVectorizer is not None:
        try:
            vectorizer = TfidfVectorizer(ngram_range=(1, 2), stop_words=list(STOPWORDS), max_features=40)
        ...
        return [
            {
                ...
                "keywords": [item["term"] for item in keyword_frequencies[:3]],
            }
...
def analyze_transcript(text: str) -> dict[str, Any]:
    ...
    if not dominant_themes:
        dominant_themes = [
            {"theme": CATEGORY_LABELS[key], "count": int(count)}
            for key, count in category_counts.most_common(TOP_THEME_LIMIT)
            if count > 0
        ]
